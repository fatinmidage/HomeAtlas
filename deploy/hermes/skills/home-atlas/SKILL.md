---
name: home-atlas
description: "家庭物品管理。读/模糊查询用 home_atlas(request)，明确写入用结构化 MCP 工具；位置先用 list_locations 校验，缺失时先 create_location，再 add_item/update_item/move_item/discard_item。当用户说\"把X放进Y\"\"X在哪\"\"上次谁动了X\"\"哪些药/卡快过期\"等家庭物品类请求时使用。Household inventory: where things are stored, find items, audit who moved them, expiry/renewal reminders."
version: 1.3.0
author: HomeAtlas
license: MIT
...
- **用前先确认工具已注册**：在你的可用工具列表中查找 `mcp_home_atlas_home_atlas`，以及 `mcp_home_atlas_list_locations` / `mcp_home_atlas_create_location` / `mcp_home_atlas_add_item` / `mcp_home_atlas_search_items` 等结构化工具。看到了再开始用。
- 本 skill **自包含**，无需加载 `native-mcp`、`mcporter` 或任何其它 skill。

## 如何调用（Workflow）

1. **先判断工具是否已注册**：在你的可用工具列表中查找 `mcp_home_atlas_home_atlas`、`mcp_home_atlas_list_locations` 和结构化写工具。MCP 工具连接的是生产 PostgreSQL 数据库，返回的数据是权威来源。不要因为结果"看起来可疑"就用 `terminal` 查本地文件验证——本地文件和生产数据库是隔离的。
2. **读/模糊请求用 `home_atlas(request)`**。例如查位置、查审计、查临期、开放式搜索：
   ```
   home_atlas(request="护照在哪？")
   ```
3. **明确写入优先用结构化工具**，这样能拿到写入后的结构化快照并核对结果：
   - 位置清单：写入前先 `list_locations()`，确认 `location_name` 已存在。
   - 新建位置：如果用户明确要用一个新位置，先 `create_location(name, parent_name?, notes?)`；如果像是错别字或近似位置，先询问用户。
   - 新增物品：确认位置存在后，再 `add_item(name, kind, location_name, ...)`。
   - 移动物品：先 `search_items(query=...)` 取 `item_id`，确认目标位置存在后，再 `move_item(item_id, location_name)`。
   ...
   - 位置管理：`rename_location(name, new_name)`、`update_location(name, parent_name?, notes?)`、`delete_location(name)`；删除位置前必须确认，且位置下不能有物品或子位置。
...
- **写入前先校验位置**。`add_item` / `move_item` 不会自动创建不存在的位置；目标位置不存在时，先用 `create_location` 显式创建，或向用户确认是不是写错了。
- **不要把结构化工具用于开放式读问题**。查位置、查审计、临期清单、模糊搜索仍可用 `home_atlas(request)`；只有明确写入才优先走 `list_locations` / `create_location` / `add_item` / `move_item` / `update_item` / `discard_item`。
...
| 用户说 | 你调用 | 期望结果 |
|---|---|---|
| 把护照放进保险柜抽屉 | `list_locations()` → 若位置存在，`add_item(name="护照", kind="document", location_name="保险柜抽屉")` | 返回写入后的物品快照 |
| 把护照放进新建的旅行文件夹 | `create_location(name="旅行文件夹")` → `add_item(name="护照", kind="document", location_name="旅行文件夹")` | 先创建位置，再写入物品 |
...
| 把豆腐乳挪到冰箱 | `search_items(query="豆腐乳")` → `list_locations()` → `move_item(item_id=..., location_name="冰箱")` | 返回新位置 |
